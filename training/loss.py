...
class StyleGAN2Loss(Loss):
    def __init__(self, device, run_dir, psp_path, clipdata, ViT16_path, ViT32_path, G_mapping, G_synthesis, GS_mapping, GS_synthesis, D, augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2):
        super().__init__()
        self.device = device
        ...
        lambda_texture = 0.0  
        lambda_partial = 1.0
        if ViT16_path is not None and ViT32_path is not None:
            self.clip_loss_models = {model_name: CLIPLoss(self.device, lambda_direction=lambda_direction, lambda_patch=lambda_patch, lambda_global=lambda_global, lambda_manifold=lambda_manifold, 
                                                        lambda_texture=lambda_texture, lambda_partial=lambda_partial, clip_model=model_name, clip_model_path=ViT16_path if model_name=="ViT-B/16" else ViT32_path) 
        ...
        print("get target image from:" + str(clipdata))
