...
def generate_images(
    ...
    python generate.py --outdir=out --projected_w=projected_w.npz \\
        --network=https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada-pytorch/pretrained/metfaces.pkl
    """
    G_kwargs = dnnlib.EasyDict(class_name='training.networks_hybrid.Generator', z_dim=512, w_dim=512, mapping_kwargs=dnnlib.EasyDict(), synthesis_kwargs=dnnlib.EasyDict())
    ...
    G_kwargs.synthesis_kwargs.channel_base = D_kwargs.channel_base = int(0.5 * 32768)
    ...
    common_kwargs = dict(c_dim=None, img_resolution=256, img_channels=3)
    G = dnnlib.util.construct_class_by_name(**G_kwargs, **common_kwargs).eval()

    print('Loading networks from "%s"...' % network_pkl)
    device = torch.device('cuda')
    with dnnlib.util.open_url(network_pkl) as f:
        resume_data = legacy.load_network_pkl(f)['G_ema']
    # with dnnlib.util.open_url(network_pkl) as f:
